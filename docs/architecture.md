...
- **预算检查**（`budget.exhaustion()`）：token / 金额 / 墙钟任一越线就中止
...
| `failure_token_budget` / `failure_cny_budget` | 本轮 failure 分到的额度 |
...
`failure_cny_budget` 有一处必须用 `is None` 判定的地方：`None` 表示「不设成本闸」，
...
`failure_cny_budget` 一直是 `None`，整条成本闸不存在。
