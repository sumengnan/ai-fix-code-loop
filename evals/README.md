...
| 仓库 | 任务 | 第二轮 `repair_diff` | 第三轮 `--recount` | 旧工具面 |
|---|---|---|---|---|
| ai-harness-framework | 14 | 86%（12/14） | **79%**（11/14） | 21%（3/14） |
| ai-learning-helper | 25 | 68%（17/25） | **80%**（20/25） | 16%（4/25） |

两轮的按仓库读数**方向相反**（framework 12→11、helper 17→20），这与整体只差
一个任务是同一件事的两面：**39 个任务分不出这个量级的差别**，拆到 14 / 25
更分不出。三列并排是为了让人看见这一点，不是为了比高低。

**最新一轮里落差没了**：79% 对 80%，大仓库不再更难。

这条更正针对的是 deepseek 那轮读出的「换到更大的代码库上修复率掉了一半以上」
（43% vs 17%）。**规模从来不是障碍本身 —— 它只是放大了补丁接口的缺陷**：文件
越长，上下文行越容易抄错、表头行数越容易算错，而那正是 93% 的补丁失败原因。
换掉那个接口，两个仓库就站到了同一条线上。

当时把落差归因于「仓库大」，错的不是相关性（那是真的），是**归因**：真正起
作用的变量藏在「大仓库」这个表象之下。

一条自警：这是**观察到的相关性被一次干预推翻**，而干预只做了一次、n=39，
拆到 14 / 25 更小。别把「落差消失」再读成新的因果定论 —— 它只说明旧那条
因果说不通了。
...
### 附：deepseek 那轮的按仓库读数（2026-07-29，旧工具面）

保留原文，因为上面那条自我更正针对的正是这张表：
...
1. ~~**换到更大的代码库上，修复率掉了一半以上**，而两个区间只在 21%–37% 这一小段重叠——这是这轮里最像真信号的一条。~~
   **已被上面那条自我更正推翻**：落差是真的，归因错了。
2. **定位不是瓶颈。** 更大的那个仓库定位反而更准（57% vs 43%），修复率却低得多。卡住的不是「找不到文件」，是「改不对」。**这一条经受住了后面三轮**——定位一路 90% 上下，而修复率从 18% 走到 79%。
