...
async def verify_commit(repo: str, commit: str, base_commit: str,
    ...
    **实测（2026-07-29，本仓库 545 条测试，同一台机器上连跑四次）单次全量
    384 / 466 / 581 / 678 秒，一个候选 commit 两次全量 13~23 分钟**，而候选在
    真实仓库里占提交总数的六成以上，这是把评测规模做上去的最大阻碍。

    四次之间最大差了 76%，这不是笔误：同一台机器上的负载波动就有这么大，
    所以这里给的是**一组读数**。把它写成一个单点数字，下一个人拿自己那次的
    读数一比就会以为哪里出了问题 —— 这一条是踩出来的：先写「466 秒」，第二次
    跑出 581 改成区间，第三次 678 把区间撑破，第四次 384 又把下界捅穿了。

    日期与条数同样是**必要的**，不是啰嗦：这一处原先写的是「单次全量 171 秒
    / 两次约 6 分钟」，那是套件还小的时候测的，此后套件长到 545 条，数字过期
    了两倍多而读的人无从判断它还成不成立。一个不带时间戳的实测数字必然会再次
    过期 —— 再更新时请连同日期与条数一起改。（数量级的结论不受影响：套件只会
    更长，只会让「不跑全量」更值。）
    ...
    **红侧全量确认、绿侧只 scoped —— 这是一个有意识的不对称，不是遗漏。**
    ...
    # 在 C^ 导入失败、在 C 正常」整类候选静默丢掉。实测（2026-07-28）本仓库
