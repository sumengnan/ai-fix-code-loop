...
async def verify_commit(repo: str, commit: str, base_commit: str,
    ...
    **实测（2026-07-29，本仓库 536 条测试，同一台机器上连跑三次）单次全量
    466 / 581 / 678 秒，一个候选 commit 两次全量 16~23 分钟**，而候选在真实
    仓库里占提交总数的六成以上，这是把评测规模做上去的最大阻碍。

    三次之间最大差了 45%，这不是笔误：同一台机器上的负载波动就有这么大，
    所以这里给的是**区间**。把它写成一个单点数字，下一个人拿自己那次的读数
    一比就会以为哪里出了问题 —— 这一条是踩出来的：先写了「466 秒」，第二次
    跑出 581 改成区间，第三次又跑出 678 把区间也撑破了。

    日期与条数同样是**必要的**，不是啰嗦：这一处原先写的是「单次全量 171 秒
    / 两次约 6 分钟」，那是套件还小的时候测的，此后套件长到 536 条，数字过期
