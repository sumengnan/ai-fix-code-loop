...
async def verify_commit(repo: str, commit: str, base_commit: str,
    ...
    四个阶段：scoped 到 test_files 取红、scoped 到 test_files 取绿、scoped 到
    候选本身复跑确认、最后回到 C^ 状态跑一次全量确认。红转绿的判定只需要本次
    commit 动过的那几个测试文件，跑全量是白花时间 —— 实测中单次全量在几百秒
    量级、同一台机器上波动近一倍，而候选在真实仓库里占提交总数六成以上，这是
    把评测规模做上去的最大阻碍。

    任何一次测试没能产出报告都抛 RuntimeError（require_report=True）：任务集是
    要被反复使用的 ground truth，宁可跳过一个 commit，也不能把假任务写进去 ——
    假任务不报错，只会让所有模型的修复成功率一起变低，看起来像「模型都不行」。

    阶段 3 复跑候选用例时传的是 `adapter.make_test_id` 合成的 id，不是原生
    nodeid。无效 id 的代价不是报错而是静默：pytest 在收集阶段整轮中止，写出
    一份 tests="0" 的空报告 —— 文件存在、require_report 检查不出异常，看起来
    像「全部复跑通过」。所以不能只看 `cand - recheck.ids`，必须先用
    `recheck.ran` 确认这个用例真的被跑到过。

    **红侧全量确认、绿侧只 scoped —— 这是有意识的不对称，不是遗漏。**
    C^ 那一侧是**人造状态**（源码被回退、测试来自未来），这个组合在历史上从来
    没存在过，意外全藏在那里；C 那一侧是作者真正提交过、提交前跑过全量的状态。
    加第五个阶段会让成本退回两次全量，把提速整个抵消掉。

    代价是明确的：源仓库自身在 C 处就有顺序依赖故障时（单跑绿、全量红），它会
    通过阶段 2/3，又因为在 C^ 全量下红而通过阶段 4，于是进任务集 —— 而评测时
    跑的是全量，该用例本就红，任何模型都恒判 SAME。这是一个谁都做不出来、又不
    报任何错的假任务。症状是「新流程挖出来的任务模型普遍做不出来」，**别误读成
    模型不行**，先去源仓库确认那几个 target_test 单跑与全量下结果是否一致。
    """
    ...
    # 在 C^ 导入失败、在 C 正常」整类候选静默丢掉。实测本仓库
    # 65 个候选 commit 里 32 个新增了测试文件，那正是这一类。比例随仓库历史
    # 增长而变，站得住的结论是「这是一整类，不是零星几个」。
