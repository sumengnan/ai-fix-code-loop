...
#
# **那段文字是什么意思由状态决定，不由语法决定** —— 有待答问题时它是回答，
# 没有时它是对缺陷的补充说明（见 handle）。代价是同一句话在两种状态下含义不同，
# 缓解办法是**机器每次都回显自己采纳的是哪种解读**。
...
class IssueEvent:
    ...
    #
    # **这里只做词法，不做语义**：分类要查一次 GitHub，而授权判定是纯函数
    # （脱网可穷举），不该为了分类去碰网络。分类在 handle 里做。
    # 权限判定对两种形态完全一样 —— 回答一个问题直接决定代码怎么改，与发起
    # 一次修复同级。
    text: str = ""
...
class Decision:
    ...
    reason: str = ""
    # 拒绝要不要回帖。两种拒绝性质不同：
    # - **没人在等**（压根不是命令、是 PR 上的、是 bot 发的）→ 不回。否则每条
    #   闲聊都被怼一句「这不是命令」。
    # - **有人在等**（看起来是命令但权限不够）→ 必须出声。静默丢弃会让人以为
    #   它已经在跑了。
    notify: bool = False
