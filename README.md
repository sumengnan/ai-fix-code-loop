...
- 分支：`aifix/a1b2c3d4`
- 修复：**2 / 3**
- 成本：¥1.01（238,070 tokens，按 1 USD = 7.2 CNY 折算）
...
在 GitHub 上开一个 issue，**正文第一行写 `/aifix`**，其余部分写缺陷描述：
...
在已有 issue 下评论 `/aifix` 也能触发（用来再跑一次），`/aifix 2` 用来回答它提的问题。
...
授权只有一条原则：**触发权 = 已经能改这个仓库的人**（所有者、有写入权限的协作者、
组织仓库的成员，外加一份可选的 `AIFIX_ALLOWED_USERS` 白名单）。

因为 issue 正文会驱动模型改代码、开 PR —— 一个本来就能直接推代码的人驱动它不增加任何
新风险，而一个没有写入权限的人能驱动它，等于给了他一条**间接的写路径**。没权限的人打了
`/aifix` 会收到一条说明，不会被静默丢弃。
...
> 成本单位是**美元** —— 这四轮是换币种之前跑的。现在的报告与对比表一律渲染成 `¥`；
> 这里保留原始数字不折算，折出来的数没有一轮是真跑出来的。
...
| 「拒绝启动：设置了预算上限，但没有配置价格表」 | 配 `AIFIX_PRICE_MAP`，或改用 `AIFIX_BUDGET_TOKENS` |
