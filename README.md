...
```bash
uv run aifix mine .          --max-tasks 12 --out evals/tasks.jsonl   # 从 git history 挖任务集
uv run aifix eval evals/tasks.jsonl --label deepseek-v4-pro                  # 跑一轮
...
对比表长这样（同样是真跑出来的——2 个挖掘任务 + 2 个变异任务，用替身模型，**所以数字全是 0**：替身什么都不改。它展示的是**形状**）：

```
| 模型 | 来源 | 任务数 | 定位准确率（分数, 95%CI） | 修复成功率（分数, 95%CI） | 平均成本 | … |
|---|---|---|---|---|---|---|
| 假模型 | mined   | 2 | 0% (0/2, 95%CI 0%–66%) | 0% (0/2, 95%CI 0%–66%) | 未知（未配置 AIFIX_PRICE_MAP） | … |
| 假模型 | mutated | 2 | 0% (0/2, 95%CI 0%–66%) | 0% (0/2, 95%CI 0%–66%) | 未知（未配置 AIFIX_PRICE_MAP） | … |
```

光看这四个 0 就能读出三个刻意的设计：

- **比率后面跟着分数和 95% 置信区间。** `0% (0/2, 95%CI 0%–66%)` 里的 `66%` 在说：两个任务什么都证明不了。`1/1 = 100%` 和 `12/20 = 60%` 不该长得一样——用 Wilson 而不是 Wald，因为 `1/1` 在 Wald 下给出 `[100%, 100%]`，一个宣称"确定无疑"的区间，而"只跑了一个任务"恰恰是这张表最需要说出口的事
- **来源不混算。** 挖掘（真实分布）与人造变异（`aifix mutate`，冒烟用）分布不同，成功率平均成一个数字是错的，所以它们各占一行
- **成本那格写的是「未知」不是 `$0.0000`。** 没配价格表时成本恒为 0，而一列整齐的 `$0.0000` 会被读成"极其便宜"。这个项目为「假的 `$0.00`」栽过三次
- 表里还有一列**「可疑信号」**（删除公开符号 / 新增模块级可变状态 / 改动落在诊断嫌疑文件之外）：纯静态、零模型调用、**只标注不改判定**。修复成功率高、可疑信号也高——那是**规格套利**的指纹
...
- **545 个测试**，全绿（2026-07-29 实测。全量耗时 384 / 466 / 581 / 678 秒——同一台机器连跑四次，最大差 76%，所以这里给的是四个读数而不是一个"权威"数字；本机装了 `mvn`，Maven 那批是真跑的）。约 5,400 行实现、10,000 行测试
